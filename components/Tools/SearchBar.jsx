...
const SearchBar = ({open, setOpen}) => {
    ...
    useEffect(()=>{
        ...
        const holder =[]
           if (query === ''){
               ...
               searchData?.filter((item) => {
                   if(Object.values(item).join().toLowerCase().includes(query.toLowerCase())){
                       holder.push(item)

                       setQueryResults( Array.from(new Set(holder)))

                   }else{
                       item.productSubType.filter((sub)=>{
                           if(Object.values(sub).join().toLowerCase().includes(query.toLowerCase()) ){
                               holder.push(item)
                               setQueryResults( Array.from(new Set(holder)))
    ...
    const handleClick = (data) => {
        router.push(`/shop/category/${data.category}/${data._id}`)
        setOpen(false)
    }
                            ...
                            <Dialog.Panel className="mx-auto max-w-2xl transform divide-y divide-gray-500 divide-opacity-10 overflow-hidden rounded-xl bg-white bg-opacity-80 shadow-2xl ring-1 ring-black ring-opacity-5 backdrop-blur backdrop-filter transition-all">
                                <Combobox onChange={(item) => (window.location)}>
                                    <div className="relative">
                                        <MagnifyingGlassIcon
                                            className="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-slate-500 text-opacity-40"
                                            aria-hidden="true"
                                        />
                                        <Combobox.Input
                                            className="h-12 w-full border-0 text-center bg-white pl-11 pr-4 text-slate-800 placeholder-slate-600 font-bold focus:outline-none sm:text-sm sm:uppercase"
                                            placeholder="Search..."
                                                            ...
                                                            {({ active }) => (
                                                                <>
                                                                    {active && <CheckIcon
                                                                        ...
                                                                        aria-hidden="true"
                                                                    />}
                                                                    <span onClick={()=>handleClick(product)} className="ml-3 flex-auto truncate">{product.vendor}{' '}{product.name}</span>
