...
import ProductPageDisplay from "../Products/ProductPageDisplay";
...
const NewProductPage = ({category, productColors, productSizes}) => {
    ...
    const handleSubmit = async() => {
        ...
        try{
            const res = await axios.post(`/api/rentalProduct`,
                { ...inputs,  barcode: barCode, size: sizes, }
            );
            console.log(res)
                        ...
                        <div className={`flex flex-col w-full`} action="">
                            <div className="flex items-center  justify-center w-full  gap-10">
                                ...
                                <div className={`flex  items-center gap-2`}>
                                    <button
                                        onClick={handleClick}
                                        className={`bg-green-500 text-white uppercase py-1 px-2 rounded text-sm font-bold`}>Add Img</button>
                                </div>
                            </div>
                            <div className={`grid grid-cols-2 w-full my-4 mx-10 `}>
                                ...
                                <div className={`flex flex-col gap-2 text-sm`}>
                                    ...
                                    <div className={`flex flex-col gap-1`}>
                                        <label className={`pt-1 uppercase text-slate-400 font-bold`}>1/2 Dag Price</label>
                                        <input className={`border border-slate-400 focus:outline-0 rounded text-sm p-1 w-1/2`}
                                               onChange={handleChange} name={`halfDayPrice`}
                                               type="text"/>


                                    </div>
                                    <div className={`flex flex-col gap-1`}>
                                        <label className={`pt-1 uppercase text-slate-400 font-bold`}>Dag Price</label>
                                        <input className={`border border-slate-400 focus:outline-0 rounded text-sm p-1 w-1/2 `}
                                               onChange={handleChange} name={`fullDayPrice`}
                                               type="text"/>
