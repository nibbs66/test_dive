...
import Quantity from "../../components/Tools/Quantity";
import {useSession} from "next-auth/react";
import useUser from "../api/hooks/useUser";
import axios from "axios";
import {useRouter} from "next/router";
import GuestCartLogin from "../../components/Client/credentials/GuestCartLogin";
import CartLogin from "../../components/Client/credentials/CartLogin";
import Trash from "../../components/Tools/Trash";
    ...
    return (
        <div className='min-h-screen w-full flex flex-col  gap-5'>
            ...
            <div className="flex gap-10 px-10 mt-10 flex-col xl:flex-row w-full">
                <div
                    className={`${status === "authenticated" ? 'hidden' : 'flex xl:flex-col gap-5 items-center xl:w-1/3 text-xs whitespace-nowrap w-full text-slate-400'}`}>
                ...
                <div className='flex flex-col gap-5 w-full'>
                    ...
                    {cart?.purchaseType === 'Web-Shop' && cart?.items?.map((item, idx) => (
                        <div className={`flex flex-col md:flex-row items-center md:justify-between gap-2 md:gap-5`}
                             key={idx}>
                            <div
                                className={`hidden md:flex md:h-16 md:w-16  rounded-full object-contain overflow-hidden border-2 border-slate-200`}>
                            ...
                            <div className='flex flex-col items-center gap-3'>
                                <span
                                    className='md:text-xl text-sm md:font-thin'>€{Number(item.price * (item.quantity)).toFixed(2)}</span>
                                <Quantity trash={false} quantity={item.quantity} locator={idx} item={item}
                                ...
                                <span
                                   ...
                                   {/*<Trash height={`h-5`} width={w-5} id={id} item={item} idx={idx}
                                    ...
                                    <Trash height={`h-5`} width={`w-5`} id={id} shoppingCart={true} item={item}
                                           idx={idx}/>
                ...
                <div className="flex flex-col gap-5  md:text-sm text-xs">
                    <div className="flex gap-10 ">
                        ...
                        <div className='flex flex-col  gap-3 items-end'>
                            ...
                            <input
                                className={'border-0 border-b border-slate-500 pl-2 py-0 text-slate-500 focus:border-b-none'}
                    ...
                    <div className="flex justify-between  md:text-sm text-xs">
                        ...
                        <div className='flex flex-col  gap-3 items-end text-slate-500'>
                            ...
                            <span>€0.00</span>
                            {cart?.total > 0 ? <span>€{(cart?.total - (cart?.total / 1.21))?.toFixed(2)}</span> :
                                <span></span>}
                    ...
                    <Link href={`/checkout`} passHref>
                        <div className='relative flex flex-col justify-center group'>
                            ...
                            <Popover disabled={disabled}  message={selected.length === 0 && !guestCheck ? "Please sign in and Choose Shipping Method to Continue" : "Please choose shipping method to Continue"}/>
                        </div>
                    </Link>
